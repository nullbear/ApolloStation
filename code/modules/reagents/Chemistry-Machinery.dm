	...
	if(beaker)
		...
		if (href_list["analyze"])
			...
			while (count--)
				...
				if(src.loaded_pill_bottle)
					if(loaded_pill_bottle.storage.can_be_inserted(P))
						P.loc = loaded_pill_bottle.storage
						src.updateUsrDialog()
...
/obj/machinery/chem_master/attack_hand(mob/user as mob)
	...
	if(!beaker)
		...
		if(src.loaded_pill_bottle)
			dat += "<A href='?src=\ref[src];ejectp=1'>Eject Pill Bottle \[[loaded_pill_bottle.storage.contents.len]/[loaded_pill_bottle.storage.max_slots]\]</A><BR><BR>"
		else
